...
main1 =
    do putStrLn "regex 1:"
       ...
       putStrLn $ "data size: " ++ show (length dat * 8)
       putStrLn $ "code size: " ++ show ((length . code tags . parse tags) dat)
       print $ (flatten . decode tags . code tags . parse tags) dat

main :: IO ()
main =
  do cs <- readFile "tags.xml"
     let bs = (code tags . parse tags) cs
     print $ length bs
