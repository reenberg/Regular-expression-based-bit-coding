...
The advantage of using a self-describing data format as XML is flexibility and
acces to the vast amount of existing tools and technologies tailored at
XML. Nonetheless encoding data in XML is highly inefficient in terms of
space-usage, as large amounts of meta-data are replicated throughout the encoded
file; hence tools for compressing XML encoded data is of main concern.
...
encode the parse tree efficiently using a bitcoding technique which only encode
...
programming language semantics \cite{gu1992, wi1993}. Knowledge of Huffman
coding \cite[Section 16.3]{co2001} is necessary to understand Section
\ref{sec:convert_huffman}.

\fixme{knowledge of XML and XML shemas}
...
under regular expressions effeciently as bit sequences. This is closely related
...
section \ref{sec:convert} shows how to automatically derive a regular expression from an XML schema;
section \ref{chap:mu-recursion} extends regular expressions with recursion, and shows how to encode parse trees more efficiently;
section \ref{chap:specialize_huffman} shows how to specialise a regular expression to the specific XML-data being compressed;
...
The authors extend their graditude to Joakim Ahnfelt-Rønne, Jon Elverkilde, and
...
Based on the formalization of Henglein and Nielsen \cite{heni2010} we focus on
string parsing under regular expressions. Figure \ref{fig:inhabitation_proofs}
...
it. Proof trees $v$, such that $\vdash v : E$ for some $E$, will be refered to
...
A proof value consisting of nested alternations is refered to as a
\emph{sum}. All subtrees in a sum which is not itself a sum is refered to as a
\emph{branch}.
...
the same languages as $Reg_{\Sigma}$:\fixme{Fixed the cite, check it is the
  right one}
\begin{enumerate}
...
\begin{lemma}
  ...
  \begin{proof} (Sketch).
    ...
    with the additional rule \fixme{cite was fixed, is it correct?}
    \[
