...
schemes is necessary to understand \fref{sec:convert}, and knowledge of Huffman
...
where DBLP$^1$ is a small sample file we have constructed from the original DBLP
XML file. This sample file is roughly of size 1mb.
...
\begin{nonfloatingtable}
  \centering
  \begin{tabular}{|l|c|c|c|} \hline \textbf{Description} & \textbf{Original
      size} & \textbf{Size after gzip} & \textbf{Size after regp} \\ \hline

    DBLP$^1$ XML file  & 1069858 & 177541 & XXXX \\
    \hspace{1em} \fs{-- Split into data} & \fs{606046} & \fs{144779} & \\
    \hspace{1em} \fs{-- Split into meta} & \fs{457985} & & \fs{94745} \\
    \multicolumn{4}{|p{300px}|}{\fs When the file is split into data and meta and the data
      is compressed using gzip and the meta is compressed using our compression
      we end up with a total size of $144779+94745 = \mathbf{239524b}$. } \\ \hline
  \end{tabular}
...
We have recounted and extended the technique described in \cite{heni2010} for encoding parse trees of regular expressions. Further we have shown how to use this technique for compressing XML-files, by automatically deriving regular expressions from (non-recursive) XML schemes. Finally, we have implemented a prototype of an XML-compressor based on our approach, and conducted a test using the prototype on data taken from the Digital Bibliography Library Project.

Our approach suffers from the following drawbacks:

\begin{itemize}
    \item Existing parsing techniques for regular expressions are less efficient in time or space or both than the statistical methods used in GZIP which runs in linear time and constant space \cite{de1996}.
    \item Our method is dependent on the presence of a non-recursive XML scheme, whereas GZIP is able to compress arbitrary XML-files.
    \item In our experiment GZIP has a better compression ratio than our prototype.
\end{itemize}

We believe that domains exist which are suitable candidates for the techniques that we describe in this paper; however, based on the above mentioned facts, we conclude that XML-file compression isn't such a domain.
...
\item \fixme{we have done this already}A better compression ratio can probably be gained by preprocessing the XML
