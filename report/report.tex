...
\def\descript
{
  ...
  regular expression from an XML schema, we can compress all XML instances of
  that schema.
...
the XML format.  Given an XML schema file \cite{wa2001}, we automatically derive
a regular expression which matches all instances of the XML schema. Using this
regular expression enables us to parse an instance of the XML schema, and we
...
schemas is necessary to understand \fref[plain]{chap:convert}, and knowledge of Huffman
coding \cite[section 16.3]{co2001} is necessary to understand
\fref[plain]{chap:huffman}.
...
\emph{XMill}. By using a specialised version of \emph{zlib} \cite{dega1996} they
...
\Fref[plain]{chap:formalisation} lays down the formal foundation on which we reason about regular expressions;
\Fref[plain]{chap:convert} shows how to automatically derive a regular expression from a DTD schema;
\Fref[plain]{chap:mu-recursion} extends regular expressions with recursion, and shows how to encode parse trees efficiently;
\Fref[plain]{chap:huffman} shows how to specialise a regular expression to the specific XML-data being compressed;
\Fref[plain]{chap:experimental_results} evaluates our prototype and presents experimental results;
\Fref[plain]{chap:conclusion} concludes the paper and presents further work.
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    $\mathfrak{c_2}$ code a membership proof of $\alpha\lbrack \alpha_i / X_i
  ...
  (e.g., $| \mathfrak{b'} | = | \mathfrak{b} |$) when using such a parser.
\end{theorem}
...
We have implemented a parser, a coder and a decoder that works on closed
tail-recursive $\mu$-expression. Furthermore we have implemented the
$norm$-function (see \fref{fig:normal-form}) and the specialisation of
$\mu$-recursive regular expressions to a particular parse tree (see
\fref{chap:huffman}).
...
for the purpose. We have compared our results to the standard Unix tool GZIP
version 1.3.12.
...
compression that is roughly 37\% worse when comparing the split method and GZIP.
See \ref{tab:experimental-results-comparison} for the complete list.
...
expressions from (non-recursive) XML schemas. Finally we have implemented a
...
\item [Splitting meta and data.] It seems that statistical compression
  ...
  the meta and the data separated. It certainly does it better than our bit
